...
# FIX IT - wintri plotting works if coord_sf(datum = NULL) is included; go back to ne_countries for more straightforward code?
...
# what type of projection (World Geodetic System 1984 aka WGS84 aka EPSG 4326 - same as what's used in GPS)
st_crs(world)
...
# Different aes() how to make each country the same color, different color?
ggplot(data = world) +
  geom_sf(aes(color = admin)) +
...
```{r}
# Choose a country
kir_medium <- ne_countries(scale = "medium", returnclass = "sf", type = "countries", country = c("Kiribati", "Australia", "New Zealand", "Papua New Guinea", "Indonesia", "Solomon Islands", "New Caledonia", "Vanuatu", "Malaysia", "Philippines", "Vietnam", "Cambodia", "Thailand"))

# Use the Admin column in world object to get the "correct" name
sort(world$admin)

# For things like Alaska, Kiribati that span the Dateline
ggplot(data = kir_medium) +
  geom_sf() 

kir_medium <- st_shift_longitude(kir_medium)
# Notice change in bbox coordinates: all negative numbers are +360
...
# Or filter for a specific "state"
wash_dc <- usa %>%
...
# Careful that your different layers are using the same coordinate system
ggplot() +
  geom_sf(data = usa %>% st_shift_longitude()) +
  geom_point(data = au_point, color = "red", shape = 17, size = 5, aes(x = long, y = lat)) 
...
# EXERCISE: Plot the country/state where you were born and the GPS coordinates of the specific city where you were born + the country/state where you are now
brooklyn_point <- data.frame(long = -73.94, lat = 40.68, name = "How it started")
au_to_brooklyn <- au_point %>% 
...
ggplot() + 
  geom_sf(data = usa_48) +
  geom_point(data = au_to_brooklyn, color = "red", shape = 17, size = 3, aes(x = long, y = lat)) +
  geom_label(data = au_to_brooklyn, aes(label = name, x = long, y = lat), nudge_y = -1.2, nudge_x = 4.5) +
