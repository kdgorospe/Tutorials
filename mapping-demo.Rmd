...
# TRY THIS: how to make each country the same color vs. different color?
ggplot(data = world) +
...
# TRY THIS: summarize world data as total population per region (use column "region_wb") and plot
...
ggplot(data = world_region_pop) +
  geom_sf(aes(fill = pop_by_region))
```
...
# TRY IT: join vaccine and spring break data with map data of USA 48 states and plot
usa_48 %>%
...
usa_48 %>%
  left_join(spring_break$interest_by_region, by = c("name" = "location")) %>%
  ggplot() +
  geom_sf(aes(fill = hits), color = "black") +
  theme_map() +
...
# TRY THIS: Plot the country/state where you were born and the GPS coordinates of the specific city where you were born + the country/state where you are now
